...
`image.repository` defaults to `ghcr.io/wei-shaw/sub2api`, the GHCR package
`release.yml` publishes on every `v*` tag. The same release also pushes
`docker.io/<dockerhub-user>/sub2api` when a DockerHub account is configured.
